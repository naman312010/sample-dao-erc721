...
contract GAToken is ERC20Pausable, AccessControl{
    ...
    constructor(string memory _name, string memory _symbol, address defAdmin, address broker, address _mainCollection, address _treasury) ERC20(_name, _symbol){
        require(defAdmin != address(0), "GAToken: admin address cannot be 0");
        require(broker != address(0), "GAToken: broker address cannot be 0");
     ...
     * @param account account to burn $GA from
     * @param amount Amount of $GA to burn from account
     */
    function redeem(address account, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(amount > 0, "GAToken: Invalid token amount");
    ...
    function claim(address _recipient) external whenNotPaused {
        require(hasRole(BROKER_ROLE, msg.sender) || _recipient == msg.sender, "GAToken: Caller neither receipent nor broker");
