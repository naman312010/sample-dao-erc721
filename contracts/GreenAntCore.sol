...
contract GreenAntCore {

    address public admin;
    INFTreeCollection public mainCollection;
    IGAToken public carbonToken;
    ...
    constructor(
        ...
        // );
        // carbonToken = GAToken("$GATest", "$GAT", address(this), _carbonBroker, address(mainCollection), _treasury);
        require(
    ...
    function setGAToken(address _token) external onlyAdmin {
        require(address(carbonToken) == address(0), "GAToken already set");
        require(
            _token != address(0),
            "Token address cannot be 0"
        );
        carbonToken = IGAToken(_token);
    }
